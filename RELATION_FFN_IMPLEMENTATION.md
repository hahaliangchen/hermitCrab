...
完整训练器默认使用 1500 个关系候选、完整输入最长 256（含 `[CLS]`/`[SEP]`）、至少 12 个内容 token，
并以 `--memory-budget-mb 1024` 按进程 RSS 检查并记录峰值；隐藏状态缓存和
完整 bank 初始化使用 `torch.no_grad()`，缓存完成后释放冻结的主 BERT。超过预算会抛出
带阶段和样本位置的错误，不继续训练。该保护不能替代 block 上限，所以正式运行前仍要
先做最坏样本探针。长度策略变为 256 后，旧的 128 token 探针只作历史基线；正式运行应重新用
最长 256 token 的真实草稿探针，
